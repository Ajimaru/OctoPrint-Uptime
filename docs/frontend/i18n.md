...
- JavaScript extraction: ensure JSDoc or gettext calls in JS are discoverable by your `babel.cfg`. In this project the `scripts/generate-jsdocs.sh` generates `docs/api/javascript.md` but JS gettext strings should be included in `pybabel extract` if configured in `babel.cfg`.
...
- The repository contains helper scripts that already run extraction/compilation in CI or during development. Use the `.development` helpers or the documented `pybabel` commands above when changing strings.
